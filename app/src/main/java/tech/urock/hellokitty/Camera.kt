...
class Camera(context: Context, videoConfig: VideoConfig) {
    ...
    private lateinit var previewFrameBitmap: Bitmap

    private var frameInitOK = false
...
//    private val view = (context as MainActivity).viewFinder
//    private val surfacePr: Preview.SurfaceProvider = (context as MainActivity).viewFinder.surfaceProvider
...
//            .withEventListener(ContextCompat.getMainExecutor(context), captureListener)
            .start()
...
//                    (context as MainActivity), cameraSelector, preview, imageAnalyzer, videoCapture)
                cameraProvider.bindToLifecycle(
